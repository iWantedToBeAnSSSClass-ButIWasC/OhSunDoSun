...
class MainActivity : AppCompatActivity() {
    ...
    private val phoneAuthCallbacks by lazy {
        object : PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
            ...
            override fun onVerificationCompleted(credential: PhoneAuthCredential) {
                Toast.makeText(this@MainActivity, "인증코드가 전송되었습니다. 90초 이내에 입력해주세요", Toast.LENGTH_SHORT).show()
    ...
    private fun initButton() {
        ...
        binding.finishAuthButton.setOnClickListener {
            try {
                val phoneCredential = PhoneAuthProvider.getCredential(storedVerificationId, binding.authNumberEditText.text.toString())
    ...
    private fun startPhoneNumberVerification(phoneNumber: String) {
        ...
        binding.phoneAuthButton.run {
            text = "재전송"
            setTextColor(ContextCompat.getColor(this@MainActivity, R.color.dark_gray_333333))
            background = ContextCompat.getDrawable(this@MainActivity, R.drawable.btn_background_default)
        }
    }
